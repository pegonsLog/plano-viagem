...
export type IconName = 
  | 'plane' | 'plus' | 'chevron-right' | 'chevron-down' | 'chevron-up' | 'chevron-left'
  | 'check-circle' | 'clock' | 'calendar' | 'map-pin' | 'currency-dollar' | 'document-text'
  | 'table-cells' | 'pencil' | 'trash' | 'eye' | 'x-mark' | 'arrow-left' | 'clipboard-list'
  | 'home' | 'building' | 'truck' | 'credit-card' | 'chat-bubble' | 'exclamation-circle'
  | 'information-circle' | 'check' | 'plus-circle' | 'minus-circle' | 'arrow-path'
  | 'document-arrow-down' | 'bars-3' | 'globe-alt' | 'sun' | 'link';
  ...
  template: `
    <svg [attr.width]="size()" [attr.height]="size()" viewBox="0 0 24 24" 
      ...
      <ng-container [ngSwitch]="name()">
        ...
        <!-- Currency Dollar -->
        <ng-container *ngSwitchCase="'currency-dollar'">
        ...
        <!-- Document Text -->
        <ng-container *ngSwitchCase="'document-text'">
          <path d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/>
        </ng-container>
        <!-- Table Cells -->
        <ng-container *ngSwitchCase="'table-cells'">
        ...
        <!-- X Mark -->
        <ng-container *ngSwitchCase="'x-mark'">
        ...
        <!-- Chat Bubble -->
        <ng-container *ngSwitchCase="'chat-bubble'">
          <path d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z"/>
        </ng-container>
        <!-- Exclamation Circle -->
        <ng-container *ngSwitchCase="'exclamation-circle'">
          <path d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z"/>
        </ng-container>
        <!-- Information Circle -->
        <ng-container *ngSwitchCase="'information-circle'">
        ...
        <!-- Arrow Path (refresh) -->
        <ng-container *ngSwitchCase="'arrow-path'">
          <path d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99"/>
        </ng-container>
        <!-- Document Arrow Down (PDF) -->
        <ng-container *ngSwitchCase="'document-arrow-down'">
        ...
        <!-- Bars 3 (menu) -->
        <ng-container *ngSwitchCase="'bars-3'">
          <path d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/>
        </ng-container>
        <!-- Globe Alt -->
        <ng-container *ngSwitchCase="'globe-alt'">
        ...
        <!-- Minus Circle -->
        <ng-container *ngSwitchCase="'minus-circle'">
